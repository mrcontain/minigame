...
pub async fn websocket_handler(
    ...
    debug!("🔌 [websocket_handler] 收到 WebSocket 连接请求，参数: {:?}", paramas);
    ...
    debug!("✅ [websocket_handler] 获取到 player_id 参数: {}", player_id);
    ...
    let player_id = match player_id.parse::<i32>() {
        ...
        Err(_) => {
            error!("❌ [websocket_handler] player_id参数格式错误: {}", player_id);
    ...
    debug!("✅ [websocket_handler] 获取到 player_name 参数: {}", player_name);
    
    let player_name = match player_name.parse::<String>() {
        Ok(player_name) => {
            debug!("✅ [websocket_handler] player_name 解析成功: {}", player_name);
            player_name
        }
        Err(_) => {
            error!("❌ [websocket_handler] player_name参数格式错误: {}", player_name);
    ...
    debug!("✅ [websocket_handler] 获取到 weather_id 参数: {}", weather_id);
    ...
    let weather_id = match weather_id.parse::<i32>() {
        ...
        Err(_) => {
            error!("❌ [websocket_handler] weather_id参数格式错误: {}", weather_id);
    ...
    debug!("✅ [websocket_handler] 获取到 background_id 参数: {}", background_id);
    
    let background_id = match background_id.parse::<i32>() {
        Ok(background_id) => {
            debug!("✅ [websocket_handler] background_id 解析成功: {}", background_id);
            background_id
        }
        Err(_) => {
            error!("❌ [websocket_handler] background_id参数格式错误: {}", background_id);
            return (StatusCode::BAD_REQUEST, "background_id参数格式错误").into_response();
        }
    };
    
    debug!("🚀 [websocket_handler] 所有参数验证成功，准备升级 WebSocket 连接 - player_id: {}, room_id: {}, player_name: {}", player_id, room_id, player_name);
...
async fn handle_websocket(
    ...
    debug!("🎯 [handle_websocket] 进入 WebSocket 处理函数 - player_id: {}, room_id: {}, player_name: {}", player_id, room_id, player_name);
    
    debug!("🔍 [handle_websocket] 正在获取房间信息 - room_id: {}", room_id);
    let mut room_info = match state.inner.room_info.get_mut(&room_id) {
        Some(room) => {
            debug!("✅ [handle_websocket] 房间信息获取成功 - room_id: {}", room_id);
            room
        }
        None => {
    ...
    debug!("📝 [handle_websocket] 添加玩家到房间 - player_id: {}, player_name: {}", player_id, player_name);
    ...
    debug!("✅ [handle_websocket] 玩家添加成功，当前房间玩家数: {}", room_info.players.len());
    
    debug!("🚗 [handle_websocket] 添加车辆到房间 - car_id: {}", car_id);
    room_info.cars.push(Car {
        car_id,
        player_ids: vec![player_id],
    });
    debug!("✅ [handle_websocket] 车辆添加成功，当前房间车辆数: {}", room_info.cars.len());
    ...
    // 获取广播通道
    debug!("🔍 [handle_websocket] 正在获取广播通道 - room_id: {}", room_id);
    let room = match state.inner.room_broadcast_couple.get(&room_id) {
        Some(room) => {
            debug!("✅ [handle_websocket] 广播通道获取成功 - room_id: {}", room_id);
            room
        }
        None => {
            error!("❌ [handle_websocket] 房间广播pipeline不存在 - room_id: {}", room_id);
            return;
        }
    };
    let tx = room.0.clone();
    ...
    let ws_to_broadcast = tokio::spawn(async move {
        ...
        while let Some(Ok(msg)) = ws_stream.next().await {
            ...
            if let Message::Text(text) = msg {
                ...
                debug!("📡 [ws_to_broadcast] 准备广播消息 - player_id: {}, content: {}", player_id, content);
                match tx.send(MessageResponse { player_id, content: content.clone() }) {
                    Ok(_) => {
                        debug!("✅ [ws_to_broadcast] 消息广播成功 - content: {}", content);
                    }
                    Err(e) => {
    ...
    let broadcast_to_ws = tokio::spawn(async move {
        ...
        // 通知所有用户已登录
        debug!("📢 [broadcast_to_ws] 准备发送登录通知 - player_id: {}, content: {}", player_id, content);
        match tx_clone.send(MessageResponse { player_id, content: content.clone() }) {
            Ok(_) => {
        ...
        loop {
            ...
            match rx.recv().await {
                Ok(data) => {
                    debug!("📨 [broadcast_to_ws] 收到广播消息: player_id={}, content={}", data.player_id, data.content);
                    ...
                    match data {
                        MessageResponse { player_id, content } => {
                            ...
                            debug!("📤 [broadcast_to_ws] 准备发送消息到 WebSocket: {:?}", json_msg);
